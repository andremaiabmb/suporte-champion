...
<?php $__env->startSection('title', 'Loja'); ?>
    ...
    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-8">
      <div class="max-w-2xl">
        ...
        <div class="mt-6 flex flex-wrap items-center gap-3">
          ...
          <a href="<?php echo e(route('store.index')); ?>" class="inline-flex items-center gap-2 rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-900 px-4 py-2.5 hover:bg-slate-50 dark:hover:bg-slate-800 transition">
  ...
  <form method="GET" class="flex-1">
    <div class="flex items-center gap-2">
      <div class="relative w-full md:w-96">
        ...
        <input name="q" value="<?php echo e($query); ?>" placeholder="Buscar produtos..."
        ...
        <?php if($activeCat): ?>
          <input type="hidden" name="cat" value="<?php echo e($activeCat); ?>">
        <?php endif; ?>
  ...
  <div class="flex items-center gap-2">
    ...
    <form method="GET" class="flex items-center gap-2">
      ...
      <select name="sort" class="rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-900 text-sm px-3 py-2.5">
        <option value="new" <?php echo e($sort==='new'?'selected':''); ?>>Mais novos</option>
        <option value="price_asc" <?php echo e($sort==='price_asc'?'selected':''); ?>>Menor preço</option>
        <option value="price_desc" <?php echo e($sort==='price_desc'?'selected':''); ?>>Maior preço</option>
      </select>
    ...
    <a href="<?php echo e(route('store.cart') ?? url('/carrinho')); ?>" class="inline-flex items-center gap-2 rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-900 px-3 py-2.5 hover:bg-slate-50 dark:hover:bg-slate-800">
  ...
  <aside class="lg:col-span-3">
    <div class="rounded-2xl border bg-white dark:bg-slate-900 border-slate-200 dark:border-white/10 p-5">
      ...
      <div class="mt-4 flex flex-wrap gap-2">
        <a href="<?php echo e(route('store.index')); ?>" class="px-3 py-1.5 rounded-full text-sm border <?php echo e($activeCat? 'border-slate-200 dark:border-white/10 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800' : 'border-brand-200 bg-brand-50 text-brand-700 dark:text-brand-200 dark:bg-brand-900/20 dark:border-brand-800'); ?>">
          Todos
        </a>
        <?php $__currentLoopData = $categories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $c): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
          <a href="<?php echo e(route('store.index', ['cat'=>$c->id, 'q'=>$query, 'sort'=>$sort])); ?>"
             class="px-3 py-1.5 rounded-full text-sm border <?php echo e($activeCat===$c->id? 'border-brand-200 bg-brand-50 text-brand-700 dark:text-brand-200 dark:bg-brand-900/20 dark:border-brand-800' : 'border-slate-200 dark:border-white/10 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800'); ?>">
            <?php echo e($c->name); ?>
    ...
    <div class="mt-6 rounded-2xl border bg-gradient-to-br from-amber-50 via-white to-rose-50 dark:from-amber-900/20 dark:via-slate-900 dark:to-rose-900/20 border-slate-200 dark:border-white/10 p-5">
      <div class="flex items-start gap-3">
        ...
        <div>
          ...
          <a href="<?php echo e(route('store.index')); ?>" class="mt-3 inline-flex items-center gap-1 text-sm text-amber-700 dark:text-amber-200 hover:underline">
            Saiba mais
  ...
  <section id="grid" class="lg:col-span-9">
    ...
    <?php else: ?>
      <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
        <?php $__currentLoopData = $products; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $p): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
          <article class="group rounded-2xl border bg-white dark:bg-slate-900 border-slate-200 dark:border-white/10 overflow-hidden hover:shadow transition">
            ...
            <a href="<?php echo e(route('store.show', $p->slug ?? $p->id) ?? url('/loja/'.$p->slug)); ?>" class="block">
              <div class="aspect-[4/3] relative bg-gradient-to-br from-slate-100 to-slate-50 dark:from-slate-800 dark:to-slate-900">
                <?php if(!empty($p->thumbnail_url)): ?>
                  <img src="<?php echo e($p->thumbnail_url); ?>" alt="<?php echo e($p->title); ?>" class="absolute inset-0 w-full h-full object-cover">
                <?php else: ?>
                  <div class="absolute inset-0 flex items-center justify-center">
            ...
            <div class="p-5">
              <div class="flex items-start justify-between gap-3">
                <h3 class="font-medium leading-tight line-clamp-2">
                  <a href="<?php echo e(route('store.show', $p->slug ?? $p->id) ?? url('/loja/'.$p->slug)); ?>" class="hover:underline">
                    <?php echo e($p->title); ?>
                ...
                <form action="<?php echo e(route('store.cart.add', $p->id ?? null) ?? url('/carrinho/adicionar')); ?>" method="POST" class="shrink-0">
                  <?php echo csrf_field(); ?>
              ...
              <div class="mt-3 flex items-baseline gap-2">
                <div class="text-lg font-semibold">R$ <?php echo e(number_format($p->price, 2, ',', '.')); ?></div>
                <?php if($p->compare_at_price && $p->compare_at_price > $p->price): ?>
                  <div class="text-sm text-slate-500 line-through">R$ <?php echo e(number_format($p->compare_at_price, 2, ',', '.')); ?></div>
                <?php endif; ?>
              </div>

              
              <div class="mt-4 flex items-center gap-2">
                <a href="<?php echo e(route('store.show', $p->slug ?? $p->id) ?? url('/loja/'.$p->slug)); ?>"
                ...
                <a href="<?php echo e(route('store.cart') ?? url('/carrinho')); ?>"
      ...
      <div class="mt-8 flex items-center justify-center gap-2">
        <a class="inline-flex items-center h-9 px-3 rounded-lg border border-slate-200 dark:border-white/10 hover:bg-slate-50 dark:hover:bg-slate-800" href="<?php echo e(request()->fullUrlWithQuery(['page'=>max(1,(int)request('page',1)-1)])); ?>">
          <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M15 18l-6-6 6-6"/></svg>
        </a>
        <span class="text-sm text-slate-500 dark:text-slate-400 px-3 py-2 rounded-lg border border-slate-200 dark:border-white/10">Página <?php echo e((int)request('page',1)); ?></span>
        <a class="inline-flex items-center h-9 px-3 rounded-lg border border-slate-200 dark:border-white/10 hover:bg-slate-50 dark:hover:bg-slate-800" href="<?php echo e(request()->fullUrlWithQuery(['page'=>(int)request('page',1)+1])); ?>">
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\Maia\Projetos\Suporte_Champion\suporte-champion\resources\views/store/index.blade.php ENDPATH**/ ?>