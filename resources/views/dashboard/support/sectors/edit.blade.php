...
@section('title','Editar Setor • Suporte')
...
<div class="space-y-6">
    ...
    <div>
      <nav class="text-xs text-slate-500 dark:text-slate-400 mb-1" aria-label="Breadcrumb">
        <ol class="inline-flex items-center gap-1">
          ...
          <li class="text-slate-600 dark:text-slate-300">Editar</li>
        </ol>
      </nav>
      <h1 class="text-xl font-semibold">Editar Setor</h1>
      <p class="text-sm text-slate-600 dark:text-slate-300">Atualize dados e equipe do setor.</p>
    </div>
  ...
  <!-- Form -->
  <form method="POST"
        action="{{ route('admin.sectors.update', $sector) }}"
        class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-white/10 p-5">
    @csrf
    @method('PUT')

    @include('dashboard.support.sectors._form', ['sector' => $sector])
