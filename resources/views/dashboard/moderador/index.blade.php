...
@section('title', 'Moderador • Dashboard')
...
<div class="mt-6 rounded-3xl shadow-soft bg-gradient-to-br from-brand-50 via-white to-sky-50 dark:from-slate-900 dark:via-slate-900 dark:to-slate-800 p-8 md:p-10">
  <div class="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
    <div>
      <h1 class="text-2xl md:text-3xl font-semibold tracking-tight">Painel do Moderador</h1>
      <p class="mt-3 text-slate-600 dark:text-slate-300 leading-relaxed">
        Revise conteúdo e acompanhe publicações e chamados.
      </p>
    </div>
...
@php
  ...
  mt_srand(crc32('moderador'.date('Y-m-d')));
  $values = array_map(fn() => max(0, 11 + mt_rand(-8, 8)), range(1,7));
@endphp
...
<div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 mt-6">
  ...
  <div class="rounded-2xl border bg-white dark:bg-slate-900 border-slate-200 dark:border-white/10 p-5">
    ...
    <div class="mt-4">
      ...
      <div class="text-sm text-slate-500 dark:text-slate-400">A revisar</div>
    </div>
  </div>
  ...
  <div class="rounded-2xl border bg-white dark:bg-slate-900 border-slate-200 dark:border-white/10 p-5">
    ...
    <div class="mt-4">
      ...
      <div class="text-sm text-slate-500 dark:text-slate-400">A responder</div>
    </div>
  </div>
  ...
  <div class="rounded-2xl border bg-white dark:bg-slate-900 border-slate-200 dark:border-white/10 p-5">
    <div class="flex items-center justify-between">
      <div class="flex items-center gap-2">
        <span class="inline-flex p-2.5 rounded-xl bg-sky-100 dark:bg-sky-900/40">
          <i data-lucide="users" class="w-5 h-5 text-sky-700 dark:text-sky-200"></i>
        </span>
    ...
    <div class="mt-4">
      <div class="text-3xl font-semibold text-slate-900 dark:text-white">{{ $metrics['users'] }}</div>
      <div class="text-sm text-slate-500 dark:text-slate-400">No sistema</div>
    </div>
  </div>
</div>
...
<div class="mt-6 rounded-2xl border bg-white dark:bg-slate-900 border-slate-200 dark:border-white/10 p-6">
  <div class="flex items-center justify-between mb-4">
    <div class="flex items-center gap-2">
      ...
      <h3 class="font-medium text-slate-700 dark:text-slate-100">Fluxo semanal</h3>
    </div>
  </div>

  <div class="relative h-48">
    <canvas id="miniChartModerador" class="w-full h-full"></canvas>
  </div>
  ...
  <script>
    (function () {
      ...
      const el = document.getElementById('miniChartModerador'); if (!el) return;
