...
<div class="mt-6 rounded-3xl shadow-soft bg-gradient-to-br from-brand-50 via-white to-sky-50 dark:from-slate-900 dark:via-slate-900 dark:to-slate-800 p-8 md:p-10">
  <div class="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
    <div>
      <h1 class="text-2xl md:text-3xl font-semibold tracking-tight">Painel do Professor</h1>
      <p class="mt-3 text-slate-600 dark:text-slate-300 leading-relaxed">
        Gerencie conteúdos e acompanhe engajamento.
      </p>
    </div>
    ...
    <div class="flex items-center gap-2">
      <a href="{{ route('admin.posts.index') }}"
        ...
        <i data-lucide="newspaper" class="w-4 h-4 text-slate-700 dark:text-slate-200"></i>
        <span>Gerir Posts</span>
      </a>
      <a href="{{ route('admin.faqs.index') }}"
         class="inline-flex items-center gap-2 px-3 py-2 rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800 transition">
        <i data-lucide="help-circle" class="w-4 h-4 text-slate-700 dark:text-slate-200"></i>
        <span>Gerir FAQs</span>
      </a>
...
@php
  ...
  $metrics = [
    ...
    'faqs'   => $safeCount('faqs'),
    'events' => $safeCount('events') ?: $safeCount('calendar_events'),
    'alunos' => $safeCount('users'),
  ];
  $labels = ['Seg','Ter','Qua','Qui','Sex','Sáb','Dom'];
  mt_srand(crc32('professor'.date('Y-m-d')));
  $values = array_map(fn() => max(0, 10 + mt_rand(-7, 7)), range(1,7));
@endphp
    ...
    <div class="flex items-center justify-between">
      <div class="flex items-center gap-2">
        <span class="inline-flex p-2.5 rounded-xl bg-emerald-100 dark:bg-emerald-900/40">
          <i data-lucide="newspaper" class="w-5 h-5 text-emerald-700 dark:text-emerald-200"></i>
        </span>
        <h3 class="font-medium text-slate-700 dark:text-slate-100">Posts</h3>
      </div>
      <a href="{{ route('admin.posts.index') }}" class="text-sm text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200 inline-flex items-center gap-1">
        <i data-lucide="arrow-right" class="w-4 h-4"></i>
      </a>
    </div>
    <div class="mt-4">
      <div class="text-3xl font-semibold text-slate-900 dark:text-white">{{ $metrics['posts'] }}</div>
      <div class="text-sm text-slate-500 dark:text-slate-400">Publicações</div>
    </div>
  </div>
    ...
    <div class="flex items-center justify-between">
      <div class="flex items-center gap-2">
        <span class="inline-flex p-2.5 rounded-xl bg-violet-100 dark:bg-violet-900/40">
          <i data-lucide="help-circle" class="w-5 h-5 text-violet-700 dark:text-violet-200"></i>
        </span>
        <h3 class="font-medium text-slate-700 dark:text-slate-100">FAQs</h3>
      </div>
      <a href="{{ route('admin.faqs.index') }}" class="text-sm text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200 inline-flex items-center gap-1">
        <i data-lucide="arrow-right" class="w-4 h-4"></i>
      </a>
    </div>
    <div class="mt-4">
      <div class="text-3xl font-semibold text-slate-900 dark:text-white">{{ $metrics['faqs'] }}</div>
      <div class="text-sm text-slate-500 dark:text-slate-400">Perguntas</div>
    </div>
  </div>
    ...
    <div class="flex items-center justify-between">
      <div class="flex items-center gap-2">
        <span class="inline-flex p-2.5 rounded-xl bg-sky-100 dark:bg-sky-900/40">
          <i data-lucide="calendar" class="w-5 h-5 text-sky-700 dark:text-sky-200"></i>
        </span>
        <h3 class="font-medium text-slate-700 dark:text-slate-100">Eventos</h3>
      </div>
      <a href="{{ route('admin.events.index') }}" class="text-sm text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200 inline-flex items-center gap-1">
        <i data-lucide="arrow-right" class="w-4 h-4"></i>
      </a>
    </div>
    <div class="mt-4">
      <div class="text-3xl font-semibold text-slate-900 dark:text-white">{{ $metrics['events'] }}</div>
      <div class="text-sm text-slate-500 dark:text-slate-400">Cadastrados</div>
    </div>
  </div>
    ...
    <div class="flex items-center justify-between">
      <div class="flex items-center gap-2">
        <span class="inline-flex p-2.5 rounded-xl bg-amber-100 dark:bg-amber-900/40">
          <i data-lucide="users" class="w-5 h-5 text-amber-700 dark:text-amber-200"></i>
        </span>
        <h3 class="font-medium text-slate-700 dark:text-slate-100">Alunos</h3>
      </div>
      <span class="text-sm text-slate-400 dark:text-slate-500">—</span>
    </div>
    <div class="mt-4">
      <div class="text-3xl font-semibold text-slate-900 dark:text-white">{{ $metrics['alunos'] }}</div>
      <div class="text-sm text-slate-500 dark:text-slate-400">Usuários no sistema</div>
    </div>
  </div>
</div>
...
<div class="mt-6 rounded-2xl border bg-white dark:bg-slate-900 border-slate-200 dark:border-white/10 p-6">
  <div class="flex items-center justify-between mb-4">
    <div class="flex items-center gap-2">
      ...
      <h3 class="font-medium text-slate-700 dark:text-slate-100">Engajamento semanal</h3>
    </div>
  </div>

  <div class="relative h-48">
    <canvas id="miniChartProfessor" class="w-full h-full"></canvas>
  </div>
  ...
  <script>
    (function () {
      ...
      const el = document.getElementById('miniChartProfessor'); if (!el) return;
