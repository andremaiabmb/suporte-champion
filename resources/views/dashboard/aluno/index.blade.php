...
<div class="mt-6 rounded-3xl shadow-soft bg-gradient-to-br from-brand-50 via-white to-sky-50 dark:from-slate-900 dark:via-slate-900 dark:to-slate-800 p-8 md:p-10">
  <div class="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
    <div>
      <h1 class="text-2xl md:text-3xl font-semibold tracking-tight">Painel do Aluno</h1>
      <p class="mt-3 text-slate-600 dark:text-slate-300 leading-relaxed">
        Acompanhe seu progresso e acesse recursos úteis.
      </p>
    </div>
...
@php
  ...
  mt_srand(crc32('aluno'.date('Y-m-d')));
  $values = array_map(fn() => max(0, 8 + mt_rand(-6, 6)), range(1,7));
@endphp
...
<div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 mt-6">
  ...
  <div class="rounded-2xl border bg-white dark:bg-slate-900 border-slate-200 dark:border-white/10 p-5">
    <div class="flex items-center justify-between">
      ...
      <a href="{{ url('/posts') }}" class="text-sm text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200 inline-flex items-center gap-1">
        <i data-lucide="arrow-right" class="w-4 h-4"></i>
      </a>
    </div>
    <div class="mt-4">
      <div class="text-3xl font-semibold text-slate-900 dark:text-white">{{ $metrics['posts'] }}</div>
      <div class="text-sm text-slate-500 dark:text-slate-400">Materiais e novidades</div>
    </div>
  </div>
  ...
  <div class="rounded-2xl border bg-white dark:bg-slate-900 border-slate-200 dark:border-white/10 p-5">
    <div class="flex items-center justify-between">
      ...
      <a href="{{ url('/duvidas') }}" class="text-sm text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200 inline-flex items-center gap-1">
        <i data-lucide="arrow-right" class="w-4 h-4"></i>
      </a>
    </div>
    <div class="mt-4">
  ...
  <div class="rounded-2xl border bg-white dark:bg-slate-900 border-slate-200 dark:border-white/10 p-5">
    ...
    <div class="mt-4">
      ...
      <div class="text-sm text-slate-500 dark:text-slate-400">Próximos e passados</div>
    </div>
  </div>
    ...
    <div class="flex items-center justify-between">
      <div class="flex items-center gap-2">
        <span class="inline-flex p-2.5 rounded-xl bg-amber-100 dark:bg-amber-900/40">
          <i data-lucide="check-square" class="w-5 h-5 text-amber-700 dark:text-amber-200"></i>
        </span>
        <h3 class="font-medium text-slate-700 dark:text-slate-100">Tarefas</h3>
      </div>
      <span class="text-sm text-slate-400 dark:text-slate-500">—</span>
    </div>
    <div class="mt-4">
      <div class="text-3xl font-semibold text-slate-900 dark:text-white">{{ $metrics['todo'] }}</div>
      <div class="text-sm text-slate-500 dark:text-slate-400">Pendências</div>
    </div>
  </div>
</div>
...
<div class="mt-6 rounded-2xl border bg-white dark:bg-slate-900 border-slate-200 dark:border-white/10 p-6">
  <div class="flex items-center justify-between mb-4">
    <div class="flex items-center gap-2">
      ...
      <h3 class="font-medium text-slate-700 dark:text-slate-100">Progresso semanal</h3>
    </div>
  </div>

  <div class="relative h-48">
    <canvas id="miniChartAluno" class="w-full h-full"></canvas>
  </div>
  ...
  <script>
    (function () {
      ...
      const el = document.getElementById('miniChartAluno'); if (!el) return;
