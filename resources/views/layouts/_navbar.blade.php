<header class="sticky top-0 z-50 bg-brand-900/95 backdrop-blur border-b border-white/10">
  <div class="container">
    <div class="flex h-16 items-center justify-between">
      {{-- === LOGO + NOME === --}}
      <a href="{{ route('home') }}" class="flex items-center gap-2" aria-label="{{ __('nav.brand_aria') }}">
        <img
          src="{{ asset('images/champion-logo.svg') }}"
          alt="Champion Logo"
          class="h-9 w-auto"
          loading="eager"
          fetchpriority="high"
      ...
      <nav class="hidden md:flex items-center gap-6">
        ...
        {{-- ✅ Suporte aparece para usuários logados --}}
        @auth
          @php
            $user = auth()->user();
            ...
            $supportRoute = in_array($isAdmin, ['admin', 'moderador', 'staff'])
              ...
              : 'support.my.index';
          @endphp
          <a href="{{ route($supportRoute) }}"
             class="text-white/90 hover:text-white text-sm {{ request()->routeIs('support.*') || request()->routeIs('admin.supports.*') ? 'underline underline-offset-4' : '' }}">
            {{ __('nav.support') }}
          </a>
        @endauth
        ...
        @if (Route::has('store.index'))
          <a href="{{ route('store.index') }}"
             class="text-white/90 hover:text-white text-sm {{ request()->routeIs('store.*') ? 'underline underline-offset-4' : '' }}">
            {{ __('nav.store') }}
          </a>
        @endif
        ...
        @auth
          <a href="{{ route('dashboard') }}"
             class="text-white/90 hover:text-white text-sm {{ request()->routeIs('dashboard*') ? 'underline underline-offset-4' : '' }}">
            {{ __('nav.dashboard') }}
          </a>
        @endauth
      </nav>
      ...
      <div class="flex items-center gap-2">
        ...
        {{-- (form antigo removido; agora usamos fetch + CSRF) --}}
        ...
        <div class="relative" id="lang-dropdown">
          <button type="button" id="lang-btn"
            ...
            <span class="font-medium" id="lang-current">{{ strtoupper($cur) }}</span>
          ...
          <div id="lang-menu"
            ...
            <div class="py-1">
              @foreach (['pt'=>'PT','en'=>'EN','es'=>'ES','fr'=>'FR'] as $code => $lbl)
                <button type="button" data-lang="{{ $code }}" role="menuitem"
                        class="w-full text-left px-3 py-2 text-sm text-white/90 hover:bg-white/10 {{ $cur === $code ? 'bg-white/10' : '' }}">
                  {{ $lbl }}
                </button>
              @endforeach
            </div>
        ...
        @auth
          <div class="relative" id="user-dropdown">
            <button id="user-btn" type="button"
                    ...
                    aria-haspopup="menu" aria-expanded="false">
              <span class="mr-2 truncate max-w-[12ch]">{{ auth()->user()->name }}</span>
        ...
        <button id="themeToggle"
                ...
                title="{{ __('nav.theme_toggle') }}" type="button">
        ...
        <button id="menuBtn"
                ...
                type="button" aria-label="{{ __('nav.open_menu') }}">
