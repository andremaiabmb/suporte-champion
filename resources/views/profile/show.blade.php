...
<div class="mt-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
  ...
  {{-- Dados básicos (apenas exibição, mantenha seu form real se já existir) --}}
  <div class="rounded-2xl border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-900 p-6">
    ...
    <div class="mt-4 space-y-3 text-sm">
      <div><span class="text-slate-500">Nome:</span> <span class="font-medium">{{ $user->name }}</span></div>
      <div><span class="text-slate-500">Email:</span> <span class="font-medium">{{ $user->email }}</span></div>
      <div><span class="text-slate-500">ID:</span> <span class="font-medium">#{{ $user->id }}</span></div>
    </div>
  </div>
  ...
  <div class="rounded-2xl border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-900 p-6">
    <div class="flex items-center justify-between">
      ...
      <div class="text-xs text-slate-500">Código: <span class="font-mono">{{ $qrCode }}</span></div>
    </div>
